...
st.set_page_config(page_title="Curious Circle Survey V2", page_icon=":clipboard:", layout="centered")
...
def render_question():
    ...
    st.markdown(f"<p class='question-text'><span class='question-number'>{question['number']}.</span> {question['main']} <em>{question['instruction']}</em></p>", unsafe_allow_html=True)
